...
class TechnicianService {
  ...
  // Get all technicians
  static Future<List<dynamic>> getAllTechnicians() async {
    final response = await http.get('$baseUrl/technicians');
  ...
  static Future<dynamic> getTechnicianById(String id) async {
    final response = await http.get('$baseUrl/technicians/$id');
    if (response.statusCode == 200) {
  ...
  static Future<void> deleteTechnician(String id) async {
    final response = await http.delete('$baseUrl/technicians/$id');
    if (response.statusCode == 200) {
      return;
